...
export async function GET(req: NextRequest) {
  ...
  if (format === 'csv' || format === 'json') {
    ...
    const rows = (data || []).map((c) => ({
      ...
      height: c.height ?? '',
      weight: c.weight ?? '',
      dominant_foot: c.dominant_foot ?? '',
      arch_type: c.arch_type ?? '',
      callus_locations: Array.isArray(c.callus_locations) ? c.callus_locations.join(' / ') : '',
      sports_history: c.sports_history ?? '',
      practice_frequency: c.practice_frequency ?? '',
      nail_care_style: c.nail_care_style ?? '',
      nail_care_frequency: c.nail_care_frequency ?? '',
      uses_insole: c.uses_insole === true ? 'あり' : c.uses_insole === false ? 'なし' : '',
      nail_color_change: c.nail_color_change ? 'あり' : 'なし',
      nail_brittle: c.nail_brittle ? 'あり' : 'なし',
      nail_pain: c.nail_pain ? 'あり' : 'なし',
      nail_growth_change: c.nail_growth_change ? 'あり' : 'なし',
      curved_nail: c.curved_nail ?? '',
      hallux_valgus: c.hallux_valgus ?? '',
      toe_grip: c.toe_grip ?? '',
      grip_confidence: c.grip_confidence ?? '',
      balance: c.balance ?? '',
      ankle_sprain: c.ankle_sprain ?? '',
      current_pain_areas: Array.isArray(c.current_pain_areas) ? c.current_pain_areas.join(' / ') : '',
      nail_findings: Array.isArray(c.nail_findings) ? c.nail_findings.join(' / ') : '',
